...
def enter_dungeon(player_stats, inventory, dungeon_rooms, clues, artifacts):
    for room in dungeon_rooms:
        # Ensure room is a tuple of 4 elements
        if len(room) != 4:
            raise TypeError("Each dungeon room must be a tuple with 4 elements.")

        room_name, item, challenge, challenge_data = room

        # Check if challenge_data is appropriate for the challenge type
        if challenge == "puzzle" or challenge == "trap":
            if not isinstance(challenge_data, tuple) or len(challenge_data) != 3:
                raise TypeError("For 'puzzle' or 'trap', the challenge_data must be a tuple of 3 elements.")
